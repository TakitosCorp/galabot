...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addUserOption((option) =>
      option.setName("user").setDescription("The user to remind about the rules.").setRequired(false)
    )
  ...
  async execute(interaction, client, clientManager) {
    ...
    if (user) {
      ...
      try {
        ...
        await interaction.reply({ content: t.dmSuccess(user.tag), ephemeral: true });
      } catch (error) {
        discordLog("warn", t.logDmFail(user.tag));
        try {
          const channel = await client.channels.fetch(interaction.channelId);
          await channel.send({ content: t.dmFallback(user.id) });
          await interaction.reply({ content: t.dmFallbackReply(user.tag), ephemeral: true });
        } catch (channelError) {
