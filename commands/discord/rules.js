...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("rules")
    .setDescription("Envía las normas a un canal o a un usuarios.")
    .addUserOption((option) =>
      option.setName("usuario").setDescription("El usuario al que deseas advertir").setRequired(false)
    )
  ...
  async execute(interaction, client) {
    ...
    if (user) {
      ...
      try {
        ...
        discordLog(
          "error",
          `No se pudo enviar DM a ${user.tag}, intentando notificar en canal. Error: ${error.message}`
        );
        ...
        const mentionEmbed = new EmbedBuilder()
          ...
          .setTitle(`Recordatorio para ${user.username}`)
          .setDescription(
            `¡Hola <@${user.id}>! Me han pedido que te recuerde que leas la normativa, ve a <#1080660073858220147> a leerla ${galabot_galanotas}.`
          );

        await channel.send({ content: `<@${user.id}>`, embeds: [mentionEmbed] });
        discordLog("info", `Recordatorio de normas enviado al canal porque los DMs de ${user.tag} están cerrados`);
        await interaction.reply({
          content: `⚠️ No se pudo enviar DM a ${user.tag}, se notificó en el canal.`,
          ephemeral: true,
        });
      }
    } else {
      ...
      discordLog("info", `Normas del servidor enviadas por petición de ${interaction.user.username}`);
